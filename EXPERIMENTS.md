...
## 발견 2 강화 실험 [주제 피벗 2026-06-23 #2 — 발견 2가 메인이므로 결정적으로]
새 주제(외부 CPU↔GPU decode 경합 비대칭 측정 + KV 함의)를 결정적으로 만드는 실험. **우선순위 순(P1→P3). 통제 전부 기존대로: 충전기 연결·발열 통제·앱 종료·powermetrics·mx.eval·median+cooldown·set_memory_limit ~12GB. 이번 핸드오프는 spec만, 실행은 사용자가 충전기 꽂고 별도 P1부터.**

### [P1] KV 연결 강화 (논문 핵심 — "경합이 결정을 바꾼다"를 증거화)
- **α-weighted drop_gain:** drop_gain = α·recency_term + (1−α)·recompute_term (각 항 정규화). α∈[0,1] sweep, **곡선으로 보고(α 고르지 말 것)**. 실 ShareGPT 트레이스(Exp3 워크로드 재사용). 산출=정책이 α에 따라 어떻게 갈리는지.
- **oracle PCIe-cost vs UMA-cost A/B:** 동일 트레이스를 두 비용 모델(PCIe-cost: swap~19ms 보존 / UMA-cost: drop+recompute)로 oracle 각각 돌려 *다른 evict 결정*을 내리는지 측정. **경합 있음/없음 두 조건에서** → "경합 시 UMA-cost oracle이 PCIe-cost oracle과 다른 결정"을 직접 증거화.
- **contention-aware victim selection (작게, positive 한 점):** CPU 부하 신호(또는 측정된 경합 슬로다운 계수)를 victim 점수에 넣은 정책 → 경합 하에서 LRU 대비 회복되나. 단순 구현, off-ramp 유지(안 되면 negative로 기록).
- 산출: "경합이 결정을 바꾼다" + "경합 인지하면 개선" → KV 연결을 "모델링돼야"(약)에서 "결정 갈리고 개선됨"(강)으로 격상.

### [P2] 현실성·견고성 (synthetic 방어 → 해결)
- **CPU 부하 종류 다양화:** STREAM 외 — memcpy loop / file·mmap scan / random access(vector-search류) / 토크나이저·임베딩 전처리류. "realistic agent-side CPU 트래픽도 decode를 때린다"를 입증(에이전트 co-execution 현실성).
- **intensity sweep 정교화:** 0 / 10 / 20 / 40 / 60 / 70 GB/s → decode slowdown 단조 곡선(예쁘게, 더 촘촘히).
- (이미 있음: prefill 면역, 크기 robust 0.5–7B, 0/50/100%) — 보강만, 재측정 불필요시 인용.

### [P3] 교차검증 (Mac mini, 나중 — artifact 방어)
- 핵심 비대칭만 재현: CPU 부하 0/50/100% vs decode/prefill + GPU 클럭 로그. 팬 있는 기기 → "발열 아님" 강화 + (칩 다르면) "M4 Air 아티팩트 아닌 UMA 일반 성질"로 일반화. *친구 Mac mini, 칩 확인 후.*

### P1 결과 [측정 2026-06-23 — `src/exp_p1_kvlink.py`, cost-model sim, 실 ShareGPT 2k convs, concurrency 16]
모든 입력 측정값: Exp1 비용곡선 + Exp2 경합계수(decode ×1.365 @풀로드, CSV서 로드). PCIe swap=19ms(모델값, 라벨). 결정성(seed 고정). 산출: results/csv/exp_p1_1.5B_20260623.csv, results/figures/exp_p1_kvlink_1.5B_20260623.png.
- **P1a (α-sweep, recency↔recompute eviction 축):** total latency spread가 예산 의존(2GB 2.1% → 32GB 375%), robust 최적 α 없음(0.0/0.1/0.4 사이 방랑); 경합은 곡선을 **균일하게** 올림(eviction 최적은 경합-robust). → recompute-cost eviction 휴리스틱이 recency를 안정적으로 못 이김 = **Phase 2a와 일관**.
- **P1b (UMA-cost vs PCIe-cost eviction, 예산 sweep) — 강한 헤드라인:** foresight oracle을 두 비용모델로 돌리면 victim의 **76–88% 불일치**. PCIe식(swap 싸→메모리만 비움=evict-largest) 정책을 UMA에서 쓰면 recompute 폭증 → penalty **+8%(2GB)→+51%(4)→+395%(8)→+6079%(16)→+12,933%(32GB)**. 예산이 적정해질수록(=실 엔진 타겟) UMA-oracle은 recompute→0(reuse 보호)인데 PCIe정책은 계속 recompute. → **단일 풀 UMA는 PCIe 비용모델이 mis-price; 자기 비용모델 필요**(발견1 swap 붕괴를 결정 결과로 격상).
- **P1c (경합 → keep-vs-recompute flip, 정직) — 음성:** 15,044개 결정 중 경합으로 뒤집힌 것 **0개**(모든 예산). keep-read 마진 3020×→2213×(effective BW 92→67 GB/s)로 좁아지나 keep이 여전히 압도(B*≈0.03 GB/s, Exp1 크로스오버와 일치). → **경합은 eviction 결정 안 바꿈**; KV 함의는 decode ITL/throughput + admission **accounting**.
- **종합(중요, 프레이밍 영향):** 페이퍼의 "KV 결정이 갈린다"는 **경합이 아니라** 비용모델 불일치(P1b)가 짊어진다. 경합(발견2/Exp2)은 측정 본체 + ITL/accounting 함의로 정직하게(eviction-flip 주장 금지). contention-aware *eviction*은 무효(P1c) → 경합의 positive는 decode throughput 쪽(P2에서).

### P2 결과 [측정 2026-06-23 — `src/exp_p2_loadtypes.py`, 실 GPU 측정, 1.5B, N=2048, AC·warmup·mx.eval·12GB·cooldown·P-core 핀]
`stream_load.c`에 메모리 접근 패턴 4종 추가(mode: stream/memcpy/scan/random; scan/random은 8-way 누산기로 메모리-bound 교정). `cpuload.py` mode 인자. Exp2 하니스(build_cache/decode_throughput) + measure.time_prefill 재사용 → Exp2와 직접 비교 가능. 산출: results/csv/exp_p2_intensity_1.5B_20260623.csv, exp_p2_loadtypes_1.5B_20260623.csv, results/figures/exp_p2_loadtypes_1.5B_20260623.png.
- **P2a (intensity sweep 정교화):** 11점(0/10/…/100%) → decode 둔화 **매끄러운 단조** 0→**40.4%** @ 0→68 GB/s. Exp2(~38%) 재현·정교화(예쁜 곡선).
- **P2b (부하 종류 다양화) — synthetic 방어 해결:** 풀로드(100% duty)서 decode 둔화 = stream **43.3%** / memcpy **40.2%** / scan **26.5%** / random **29.4%**. **prefill은 전부 +4.2~5.7%로 면역**(모든 패턴서 비대칭 robust). → **STREAM 아티팩트 아님**: copy·file-scan·vector-search 등 현실 에이전트측 CPU 트래픽이 모두 decode를 때리고 prefill(compute-bound)은 면역.
- **random gather 강한 부수 발견:** 논리 **2.4 GB/s**(latency-bound)인데 decode **29.4%** 둔화 — scan(51 GB/s)의 26.5%보다 **큼**. cache-line 증폭 + 메모리시스템 압박 → **적은 대역폭의 random access(벡터서치/RAG/임베딩 lookup)가 decode를 불균형하게 때린다** = on-device 에이전트 현실성 강화.
- **통제 정직:** throttle_ratio(스텝 spread) P2a 1.13–1.53(단조 추세는 robust, 발열 throttle 아닌 window 내 분산). prefill 면역은 powermetrics 메커니즘(Exp2 exp2_pm_probe)으로 이미 arbitration 확정 → P2는 부하-종류 robust성만 추가(메커니즘 재측정 생략, 인용).
