...
- [decision implication] **P1 측정(2026-06-23, cost-model sim, `src/exp_p1_kvlink.py`)으로 정직하게 분리됨:**
  (a) 경합(발견2)은 keep-vs-recompute eviction을 *안 뒤집는다* — P1c: 트레이스 15,044개 결정 중 flip 0개, keep 마진 3020×→2213×(경합 후에도 keep 압도). 경합의 KV 역할 = decode ITL/throughput + admission **accounting**(비용모델에 항은 넣되 eviction 결정 자체는 안 바뀜).
  (b) KV "결정이 갈린다"는 *다른 축* = 단일 풀 비용모델 자체. PCIe-cost(swap 싸다) vs UMA-cost(recompute) oracle이 victim의 **76–88% 불일치**, PCIe식 정책을 UMA에서 쓰면 **+8% → +12,933% TTFT**(예산이 적정해질수록 ↑; P1b). = 발견1(swap-tier 붕괴)을 *결정 결과*로 격상 = "단일 풀 UMA는 자기 비용모델이 필요"의 강한 증거.
  (c) recency↔recompute α축은 robust 최적 없음(P1a, ~2–375% spread budget 의존, Phase 2a 일관). → 페이퍼 KV-결정 섹션은 **P1b가 짊어지고**, 경합(Exp2)은 측정 본체 + ITL/accounting 함의로 정직하게 쓴다(완전 역전 주장 안 함).
...
- **[x] P1 측정 완료(2026-06-23, `src/exp_p1_kvlink.py`, cost-model sim, 실 ShareGPT 2k convs).** 핵심: 경합은 eviction 결정 *안 뒤집음*(P1c 0/15,044 flip, margin 3020×→2213×) → 경합 KV함의 = ITL/accounting; 진짜 결정 divergence = UMA-vs-PCIe 비용모델(P1b: victim 76–88% 불일치, penalty +8%→+12,933%, 예산↑일수록↑); α축 robust 최적 없음(P1a). → 페이퍼 KV-결정 섹션은 P1b(비용모델 불일치=swap 붕괴)가 짊어지고 경합(Exp2)은 측정 본체+ITL/accounting로 정직하게. 산출: results/csv/exp_p1_1.5B_20260623.csv, results/figures/exp_p1_kvlink_1.5B_20260623.png.
- **[x] P2 측정 완료(2026-06-23, `src/exp_p2_loadtypes.py`, 실 GPU, 1.5B N=2048).** synthetic 방어 해결: 현실 CPU 패턴 4종(stream/memcpy/scan/random)이 모두 decode 둔화(26–43%)·prefill 면역(+4~6%) → **STREAM 아티팩트 아님**. intensity sweep 정교화(11점, 매끄러운 단조 0→40.4%@68GB/s). 부수발견: random gather가 논리 2.4GB/s인데 decode 29.4% 둔화(cache-line 증폭→벡터서치류가 적은 BW로 더 때림). `stream_load.c`+mode/`cpuload.py` 확장. 산출: exp_p2_intensity/loadtypes_1.5B_20260623.csv + exp_p2_loadtypes_1.5B_20260623.png. → 약한 고리 (2) 해결.
- [ ] **다음(실험)** — [P3] Mac mini 교차검증(팬 기기, "발열 아님" 강화 + 칩 다르면 일반화). *약한 고리 (3).* (P2가 (2) 해결; P1b가 KV-결정 헤드라인 (1) 강화.)
