...
- [ ] **다음(실험)** — [P2] 현실 CPU 부하 다양화(memcpy/mmap/random/전처리류) + intensity sweep 정교화(0/10/20/40/60/70 GB/s). [P3] Mac mini 교차검증(나중). *주의: P1c가 contention-aware eviction 무효를 보였으니 경합의 positive는 eviction이 아니라 decode ITL/throughput 쪽 — P2에서 현실 부하로 decode 타격 재현이 핵심.*
